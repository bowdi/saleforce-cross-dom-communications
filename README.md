...
Even taking into account the fact there could be up to 3 events being in transferred in any one request (when using the [Dispatcher Pattern](https://devops.vitality.co.uk/confluence/display/SD/Dispatcher+Pattern) there could be a chain such as client -> dispatcher -> filter -> client) and a worst case scenario (three max's) this still does not get anywhere close to the critical threshold.
